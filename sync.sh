...
REMOTE_USER="peecbiz"
REMOTE_PATH="public_html/quantumaikido.com/"
[[ "$(hostname)" == "LAPTOP-8FVD6SBV" ]] && REMOTE_HOST="10.3.0.122"
[[ "$REMOTE_HOST" =~ ^10\. ]] && REMOTE_PATH="~/projects/quantumaikido.com/" && REMOTE_USER="kkron"
SSH_KEY="$HOME/.ssh/quantumaikido_ed25519"
SCP_KEY_ARGS=(-i "$SSH_KEY")
LOGS_DIR="${LOCAL_PATH}/logs/"
ACCESS_LOG_PATH="access-logs/quantumaikido.com.peec.biz-ssl_log"
ARCHIVE_LOG_PATH="logs/quantumaikido.com.peec.biz-ssl_log"
...
fetch_logs() {
    ...
    MONTH=$(date +"%b-%Y")
	  for MONTH in $(date +"%b-%Y") $(date -d "1 month ago" +"%b-%Y"); do
		  scp "${SCP_KEY_ARGS[@]}" "${LOG_USER}@${LOG_HOST}:~/logs/quantumaikido.com.peec.biz-ssl_log-${MONTH}.gz" "${LOGS_DIR}archive-ssl-${MONTH}.gz" 2>/dev/null
...
generate_report() {
    ...
    echo ""
    echo "========================================"
    echo "  QUANTUMAIKIDO.COM VISITOR STATISTICS"
    echo "  Generated: $(date)"
    echo "========================================"
    echo ""
    echo "SUMMARY"
